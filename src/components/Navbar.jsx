...
const Navbar = () => {
    ...
    return (
        <div ref={navContainerRef} className='fixed inset-x-0 top-4 z-50 h-16 border-none tansition-all duration-700 sm:inset-x-6'>
                ...
                <nav className="flex size-full items-center justify-between p-4">
                    ...
                    <div className="flex h-full items-center">
                        <div className="hidden md:block">
                            {navItems.map((item) => (
                                <a key={item} href={`#${item.toLowerCase()}`} className="nav-hover-btn font-general">
                                    {item}
                                </a>
                            ))}
                        </div>
